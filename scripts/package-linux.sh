...
output="$root/outputs/deskbridge-desktop_0.3.0_amd64.deb"
...
mkdir -p "$stage/DEBIAN" "$stage/opt/deskbridge" "$stage/usr/bin" \
  "$stage/usr/share/applications" "$stage/usr/share/icons/hicolor/512x512/apps" "$root/outputs"
...
cp "$root/desktop/icon.png" "$stage/usr/share/icons/hicolor/512x512/apps/deskbridge.png"
